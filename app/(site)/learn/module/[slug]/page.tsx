...
export default async function ModulePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const moduleLabIds = module.slug === 'supervised-learning'
    ? ['linear-regression', 'gradient-descent', 'logistic-regression', 'knn', 'decision-tree', 'svm', 'confusion-matrix', 'roc-pr', 'ensemble', 'overfitting']
    : module.slug === 'unsupervised-learning'
      ? ['kmeans', 'pca']
      : [];
  const moduleLabs = moduleLabIds.map((id) => labs.find((lab) => lab.id === id)).filter(Boolean);
  ...
  return (
    <main id="main-content">
      ...
      {module.slug === 'supervised-learning' ? (
        <section className="section shell module2-module-entry">
          <Module2Launchpad compact />
        </section>
      ) : null}
      ...
      <section className="section shell module-layout">
        ...
        <div className="module-main">
          ...
          <section className="session-section">
            <div className="content-heading"><span className="eyebrow">Official session sequence</span><h2>Lectures and paired practice.</h2></div>
          ...
          {moduleLabs.length ? (
            <section className="module-lab-section">
              <div className="content-heading"><span className="eyebrow">Interactive laboratories</span><h2>Test the ideas while they are still visible.</h2><p>Each experiment isolates one cause-and-effect relationship from the module.</p></div>
              <div className="module-lab-grid">
                {moduleLabs.map((lab) => lab ? (
                  <Link className={`module-lab-card module-lab-card--${lab.accent}`} href={lab.href} key={lab.id}>
                    <span>{lab.index}</span><div><small>{lab.tags.join(' · ')}</small><h3>{lab.title}</h3><p>{lab.description}</p></div><Icon name="arrow" />
                  </Link>
                ) : null)}
              </div>
              {module.slug === 'supervised-learning' ? <CourseLabRepository compact /> : null}
            </section>
          ) : null}
