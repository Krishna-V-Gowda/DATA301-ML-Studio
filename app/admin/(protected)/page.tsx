      ...
      <div className="admin-dashboard-grid">
        ...
        <section className="admin-card">
          ...
          <div className="attention-list">
            ...
            {firstPending ? <article><span>Next</span><div><strong>Prepare Lecture {String(firstPending.number).padStart(2, '0')}</strong><p>{firstPending.title}</p></div></article> : <article><span>Done</span><div><strong>All sessions completed</strong><p>The full official session sequence is marked complete.</p></div></article>}
        ...
        <section className="admin-card admin-card--wide">
          ...
          {recent?.length ? <div className="recent-materials">{recent.map((item) => { const released = item.status === 'scheduled' && item.publish_at && new Date(item.publish_at) <= new Date(); const effectiveStatus = released ? 'published' : item.status; return <div key={item.id}><span className={`file-kind file-kind--${item.kind}`}>{item.kind.slice(0, 2).toUpperCase()}</span><div><strong>{item.title}</strong><small>Version {item.version} · {item.file_name}</small></div><span className={`status-badge status-badge--${effectiveStatus}`}>{released ? 'published · scheduled' : item.status}</span></div>; })}</div> : <div className="empty-state-inline">No managed database materials yet. The three packaged source files remain available on the student site.</div>}
        </section>
      </div>
    </>
  );
