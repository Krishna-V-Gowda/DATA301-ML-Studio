...
export async function getPublishedModules() {
  ...
  // The typed module details remain source-controlled. Database rows currently
...
export async function getPublishedMaterials(): Promise<Material[]> {
  if (!isSupabaseConfigured) return latestPublishedMaterials(fallbackMaterials);
  ...
  if (error || !data?.length) {
    // The public package includes author-created fallback files for development and recovery,
    ...
    return process.env.VERCEL ? [] : latestPublishedMaterials(fallbackMaterials);
  }
  ...
  return latestPublishedMaterials(process.env.VERCEL ? databaseMaterials : [...fallbackMaterials, ...databaseMaterials]);
}
